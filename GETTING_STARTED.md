...
- **OpenSSL**: For generating ECDSA P-256 key pairs (issuer/principal keys for `issue-credential` and `issue-spend-authorization`) — usually pre-installed on Linux/macOS
...
### Issue a Spend Authorization (Agent Commerce)

Lets a principal (human/user) grant an agent a constrained authorization to spend on their behalf. Requires an ECDSA P-256 key pair for the principal — generate one with `openssl` if you don't already have one:

```bash
openssl ecparam -genkey -name prime256v1 -noout -out principal.pem
openssl ec -in principal.pem -pubout -out principal_pub.pem
```
...
This prints a signed JWT credential. `--max-amount` is a **per-transaction** cap on a reusable authorization (not single-use, not a cumulative budget) — the same credential can be checked against many transactions.

### Verify a Spend Authorization Against a Transaction

No server required — this works entirely offline against a credential and a public key:

```bash
cargo run -- verify-spend-authorization \
  --credential "<JWT from above>" \
  --issuer-public-key principal_pub.pem \
...
The same check is available over HTTP once the server is running, via `POST /api/v1/agent-auth/verify-transaction` — see [API.md](API.md#verify-spend-authorization--transaction) for the request/response shape. See also [ARCHITECTURE.md](ARCHITECTURE.md#agent-authorization) for how issuance (CLI-only, private-key-based) and verification (HTTP-safe, public-key-based) are split.
