...
/// Validate a generic identifier (merchant_id, category, action_id, authorization_ref)
pub fn validate_identifier(id: &str, max_len: usize) -> bool {
    !id.is_empty()
        && id.len() <= max_len
        && id.chars().all(|c| c.is_alphanumeric() || "-_.".contains(c))
}

/// Validate an ISO 4217-shaped currency code (format only, not a real currency registry)
pub fn validate_currency(code: &str) -> bool {
    code.len() == 3 && code.chars().all(|c| c.is_ascii_alphabetic())
}

/// Sanity-bound a monetary amount before it reaches constraint-checking logic
pub fn validate_amount(amount: f64) -> bool {
    amount.is_finite() && amount > 0.0 && amount < 1_000_000_000.0
}
